...
///api keys for currency converter api
const myHeaders = new Headers();
myHeaders.append("apikey", "Z6LEWsR6Zx5mQqZNf4OGvAcVSYaVS3cJ");
...
const Invoice = () => {
    ...
    useEffect(()=>{
        fetch("https://api.apilayer.com/exchangerates_data/latest?symbols=PKR&base=USD", requestOptions)
        .then(response => response.json())
        .then(result =>{
             setUsdValue(result.rates.PKR)
        })
  ...
  const handleRates =(e) => {
    ...
    useEffect(() => {
            //Get the subtotal
            const subTotal =()=> {
            var arr = document.getElementsByName("amountPKR");
            var subtotal = 0;
    ...
    <div>
        ...
        <Table className={classes.table} aria-label="simple table">
            ...
            {invoiceData.items.map((itemField, index) => (
                ...
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="text" name="batchNo" onChange={e => handleChange(index, e)} value={itemField.batchNo} placeholder="0" /> </TableCell>
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="text" name="packSize" onChange={e => handleChange(index, e)} value={itemField.packSize} placeholder="0" /> </TableCell>
                ...
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="number" name="amountUSD" onChange={e => handleChange(index, e)}  value={usdValue > 0 && (itemField.quantity * itemField.unitPrice)/usdValue - ((itemField.quantity * itemField.unitPrice) * itemField.discount / 100)/usdValue} disabled /> </TableCell>
                <TableCell align="right"> <InputBase sx={{ ml: 1, flex: 1 }} type="number" name="amountPKR" onChange={e => handleChange(index, e)}  value={(itemField.quantity * itemField.unitPrice) - (itemField.quantity * itemField.unitPrice) * itemField.discount / 100} disabled /> </TableCell>
